...
- 交互：开场居中头像 + 网名（无提示文字）→ 点击后头像右移、左侧浮现菜单（WELCOME / OPTION II / OPTION III，选项内容未定）
- 视觉：整体如像素画（非游戏 UI）；像素素材代码生成（背景场景类）**曾**全部代码生成，但头像已改为用户提供的照片 `avatar.jpg`（2026-08-27 用户决定）
- 背景：**待重做**——当前为临时 `#2E3A58` 纯色（用户指示全部推倒重来并加深，2026-08-27），原黄昏雪山场景代码已删除；重做方案未定
- 配色：**待定**——当前 :root 变量是临时占位；`--panel` 已提亮为 `#4E6496` 避免与背景同色
- 头像：`avatar.jpg`（用户提供照片，`object-fit: cover` 显示）；网名：**fghmnst**（已定）
